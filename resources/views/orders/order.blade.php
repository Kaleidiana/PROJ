    ...
    public function create(Request $request, Car $car)
{
    ...
    public function success(Request $request, Car $car)
    {
        ...
        if ($paypalResponse->successful()) {
            ...
            Order::create([
                'user_id' => auth()->id(),
                'car_id' => $car->id,
                'quantity' => 1, // Adjust based on quantity input
            ...
            return redirect()->route('cars.index')->with('error', 'Payment could not be verified.');
        }
    }
    ...
    {
        return view('order.confirmation');  // Make sure to create this view
    }
}
